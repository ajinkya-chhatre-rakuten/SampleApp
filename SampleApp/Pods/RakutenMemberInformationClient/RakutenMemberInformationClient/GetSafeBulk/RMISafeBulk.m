...
static RMISafeBulk *create_result_from_json(NSDictionary *JSON)
{
    @try
    {
        RMISafeBulk *profile = [[RMISafeBulk alloc] init];
        profile.openID = [RWCParserUtilities stringWithObject:JSON[@"openId"]];
        profile.pointAccount = [RWCParserUtilities stringWithObject:JSON[@"pointAccount"]];
        profile.addressCountryCode = [RWCParserUtilities stringWithObject:JSON[@"countryCode"]];
